...
class KDTree
{
  ...
  private:
    ...
    vector<Point<Dim>> all_points;

    void destroy(KDTreeNode *subroot);

    KDTreeNode * createTree(int left, int right, int curDim);

    void quickSelect(int begin, int end, int pivot_index, int curDim);

    Point<Dim> findNearestNeighbor(const Point<Dim>& query,typename KDTree<Dim>::KDTreeNode* currentNode, int curDim) const;
